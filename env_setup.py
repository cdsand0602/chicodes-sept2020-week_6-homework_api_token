...
def add_to_env(length):
    """
        add_to-env will create environment variables
        that will live inside of the .env file when the
        script is run
    ...
    with open('.env','w+') as file:
        database_url = 'postgresql+psycopg2://{user}:{pw}@{url}/{db}'.format(user='postgres',pw='Its@secret1',url='127.0.0.1:5432',db='api_patient_api_db')
        data = file.read()
        file.seek(0)
